...
CREATE TABLE `orders` (
  ...
  PRIMARY KEY (`OrderID`),
  KEY `fk_CustomerID_Customers` (`CustomerID`),
  KEY `fk_EmployeeID_Employees` (`EmployeeID`),
  KEY `fk_ShipperID_Shippers` (`ShipperID`),
...
CREATE TABLE `products` (
  ...
  KEY `fk_CategoryID_Categories` (`CategoryID`),
  KEY `fk_SupplierID_Suppliers` (`SupplierID`),
  CONSTRAINT `fk_CategoryID_Categories` FOREIGN KEY (`CategoryID`) REFERENCES `categories` (`CategoryID`),
  CONSTRAINT `fk_SupplierID_Suppliers` FOREIGN KEY (`SupplierID`) REFERENCES `suppliers` (`SupplierID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2022-06-05  8:19:10
